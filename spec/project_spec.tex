...
However the single-bit synchronizer only works with 1-bit signals and not a 12-bit bus like the duty cycle input.
We will implement a 4 phase handshake to make sure we reliably pass data from the RISC-V core's clock domain to the PWM block's clock domain.
...
\begin{enumerate}
    ...
    \item Acknowledge is de-asserted in the RX domain
\end{enumerate}
...
You should modify \verb|z1top.v| to wire the PWM controller's output to \verb|aud_pwm|.

\textit{Note: clarify PLL clocking connections, clarify IOB register packing, testing}
...
%% Sine NCO gain (sine, square, saw, triangle, noise)
%% Square NCO gain
%% Triangle NCO gain
%% Saw NCO gain
%% SVF F coeff
