...
\textit{Fall 19 Note:} We will be providing a test harness for running the \verb|riscv-tests| ISA test suite soon.
...
\textit{Fall 19 Note:} We are replacing \verb|coe_to_serial| with another script we will release soon.
...
For example, before making the \verb|mmult| program you should have set the set the base address to \verb|0x10006000| (see \ref{mmult}).
Therefore, when loading the \verb|mmult| program to the FPGA you should place it into the memory that it starts aligned with the base address: \verb|coe_to_serial mmult.coe 30006000|.
Then, you can start in in your screen session by using \verb|jal 10006000|.
...
\begin{enumerate}
  ...
  \item \textit{Run matrix multiply.} Load the \verb|mmult| program with the \verb|coe_to_serial| utility, and run \verb|mmult| on the FPGA. Verify that it returns the correct checksum.
...
\textbf{Fall 19 Note: This section will be updated}

We have provided a program to test your tone generator and its memory map.
It can be found in \verb|software/tone_gen_test/|.
Compile and run the program just as you did for the \verb|user_io_test|.
\verb|jal| to the program from the BIOS, and you can play with these commands:
...
The optimization goal for this project is to achieve maximal performance on the \verb|mmult| program, as defined by the 'Iron Law' of Processor Performance.
